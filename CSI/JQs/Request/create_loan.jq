    ...
    }) as $officerBorrower |
{
    ...
    collateral1Code: ((.loan_relations[] | select(.is_primary_borrower == true) | if(any(.collaterals[]; .collateral_type_verbose == "All Assets" or .collateral_type_verbose == "all_assets")) then $collateralCode["collateral1Code"] else ((if .collaterals | length > 0 then (.collaterals[0
    ] | (if .category then $collateralCode[.category
    ] else $collateralCode[.collateral_type_verbose
    ] end)) else null end)) end ) // null),
    collateral2Code: ((.loan_relations[] | select(.is_primary_borrower == true) | if(any(.collaterals[]; .collateral_type_verbose == "All Assets" or .collateral_type_verbose == "all_assets")) then $collateralCode["collateral2Code"] else ((if .collaterals | length > 1 then (.collaterals[1
    ] | (if .category then $collateralCode[.category
    ] else $collateralCode[.collateral_type_verbose
    ] end)) else null end)) end ) // null),
    collateral3Code: ((.loan_relations[] | select(.is_primary_borrower == true) | if(any(.collaterals[]; .collateral_type_verbose == "All Assets" or .collateral_type_verbose == "all_assets")) then $collateralCode["collateral3Code"] else ((if .collaterals | length > 2 then (.collaterals[2
    ] | (if .category then $collateralCode[.category
    ] else $collateralCode[.collateral_type_verbose
    ] end)) else null end)) end ) // null),
    collateral4Code: ((.loan_relations[] | select(.is_primary_borrower == true) | if(any(.collaterals[]; .collateral_type_verbose == "All Assets" or .collateral_type_verbose == "all_assets")) then $collateralCode["collateral4Code"] else ((if .collaterals | length > 3 then (.collaterals[3
    ] | (if .category then $collateralCode[.category
    ] else $collateralCode[.collateral_type_verbose
    ] end)) else null end)) end ) // null),
    ...
    customerRelationships: [if any(.loan_relations[]; select(.borrower_position == 2)) == false then (.loan_relations[] | if(.is_primary_borrower == true) then {
