...
{ 
    ...
    loanType: ( .product.product_code as $productCode | .loan_approval.approved_rate_type as $rateType |
        if ($rateType == "Fixed")
            then if ($productCode == "7A_TL" and .loan_amount>350000 )
                    then "SBA 7(a)"
    ...
    contacts: [ .loan_contacts[] as $loan_relations | 
        { 
            ...
            memberOf: ( if ($loan_relations.entity_type == "sole_proprietor" and $loan_relations.relation_type == "borrower") then [ 
                ...
                { 
                    entityName: (if $loan_relations.memberof then $loan_relations.memberof else empty end),
